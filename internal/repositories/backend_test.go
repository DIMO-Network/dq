...
// Compile-time interface assertions, duplicated from backend.go/shadow.go so
// a regression shows up as a test-file build failure with a clear location.
var (
	_ Backend   = (*ch.Service)(nil)
	_ Backend   = (*duck.Queries)(nil)
	_ CHService = (*ch.Service)(nil)
	_ CHService = (*ShadowBackend)(nil)
...
func TestComposeBackendRouting(t *testing.T) {
	...
	signalBackend := &fakeBackend{
		getEventCounts: func(context.Context, string, time.Time, time.Time, []string) ([]*ch.EventCount, error) {
			return []*ch.EventCount{{Name: "tripStart", Count: 2}}, nil
		},
	}
